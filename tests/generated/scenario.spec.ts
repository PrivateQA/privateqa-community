import { test, expect, fwkStep } from "../_fwkTest/base";

test("Scenario: A - Ouvrir https://gulafront.vercel.app;", async ({ page }, testInfo) => {
  test.setTimeout(60_000);
  await fwkStep(testInfo, page, 1, "Ouvrir https://gulafront.vercel.app", async () => {
    await page.goto("https://gulafront.vercel.app");
  });
  await fwkStep(testInfo, page, 2, "Clique sur \"Donnée d'exemple\"", async () => {
  ...
  await fwkStep(testInfo, page, 3, "Vérifier que le texte \"Résultats de l'analyse\" est visible", async () => {
    await expect(page.locator("*:visible", { hasText: /Résultats\s+de\s+l['’]analyse/ }).first()).toBeVisible({ timeout: 15000 });
  });
  await fwkStep(testInfo, page, 4, "Clique sur \"Voir plus\" (première occurence)", async () => {
  ...
  await fwkStep(testInfo, page, 5, "Vérifie la présence du text \"Recommandation\"", async () => {
    await expect(page.locator("*:visible", { hasText: /Recommandation/ }).first()).toBeVisible({ timeout: 15000 });
  });
  await fwkStep(testInfo, page, 6, "scroll jus'au text \"Avertissement médical\" pour en vérifier la présence", async () => {
  ...
  await fwkStep(testInfo, page, 7, "scroll en haut de la page", async () => {
    await page.evaluate(() => window.scrollTo({ top: 0, left: 0, behavior: "instant" as ScrollBehavior }));
  });
  await fwkStep(testInfo, page, 8, "click sur le text \"Nouvelle analyse\"", async () => {
  ...
  await fwkStep(testInfo, page, 9, "vérifie si le text \"Pourquoi Choisir Gula\" est présent", async () => {
